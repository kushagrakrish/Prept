...
import { formatDate, formatDuration, formatTime } from "@/lib/herlpers";
        ...
        <div className='flex items-start justify-between gap-4'>
          <div className='flex items-center gap-4 min-w-0'>
            <Avatar className='w-14 h-14 border border-white/10 rounded-2xl shrink-0'>
              ...
              />
              <AvatarFallback className='rounded-2xl bg-violet-500/10 border border-violet-500/20 text-violet-400 text-lg font-medium'>
                {person?.name?.[0] ?? "?"}
              </AvatarFallback>
            </Avatar>
              ...
              {mode === "interviewee" && person?.categories?.length > 0 && (
                <div className='flex flex-wrap gap-1 mt-1'>
                  {person.categories.slice(0, 3).map((cat) => (
                    <span
                      key={cat}
                      className='text-[10px] px-2 py-0.5 rounded-md border border-violet-500/20 bg-violet-500/5 text-violet-400 leading-tight'
                    >
                      {cat.replace("_", " ")}
                    </span>
                  ))}
              ...
              (has?.({ plan: "starter" }) || has?.({ plan: "pro" })) && (
                <>
                  <Button
                    variant='outline'
                    size='sm'
                    className='gap-1.5 border-violet-500/20 text-violet-400 hover:bg-violet-500/10 hover:border-violet-500/40'
